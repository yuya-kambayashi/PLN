...
namespace BaseCAD.Graphics
{
    public abstract class GDIRenderer : Renderer
    {
        ...
        private Pen CreatePen(Style style)
        {
            ...
            {
                var pen = new Pen(Color.FromArgb((int)StyleOverride.Color.Argb), GetScaledLineWeight(StyleOverride.LineWeight));
                pen.DashStyle = (System.Drawing.Drawing2D.DashStyle)StyleOverride.DashStyle;
                return pen;
            }
            else
            {
                var pen = new Pen(Color.FromArgb((int)style.Color.Argb), GetScaledLineWeight(style.LineWeight));
        ...
        private Brush CreateBrush(Style style)
        {
            if (StyleOverride != null)
            {
                return new SolidBrush(Color.FromArgb((int)StyleOverride.Color.Argb));
            }
            else
            {
                return new SolidBrush(Color.FromArgb((int)style.Color.Argb));
            }
        }
    }
}
